...
export async function POST(request: Request) {
  ...
  const body = await parseJsonBody<AcceptBaaBody>(request);
  ...
  try {
    ...
    return jsonOk({ accepted: true, version: BAA_VERSION }, 201);
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("POST /api/profile/accept-baa failed", error);
    return jsonError("Failed to record BAA acceptance", 500);
  } finally {
    client.release();
  }
}
